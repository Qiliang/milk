...
var insert_stmt = " insert into depot_outs(supplement,proxy,comment,count,shop_name,good_id,create_at,modifier,depot_id) values($supplement,$proxy,$comment,$count,$shop_name,$good_id,$create_at,$modifier,$depot_id)";
...
exports.update = function (req, res) {
    ...
    db.run(" update depot_outs set depot_id=$depot_id,supplement=$supplement,proxy=$proxy,comment=$comment,count=$count,shop_name=$shop_name,good_id=$good_id,create_at=$create_at,modifier=$modifier where id=$id", db.args(req.body)).done(function (text) {
        res.send(text);
    }, function (err) {
        res.send(500, err);
    });
...
var delete_stmt = " delete from depot_outs where id=$id";
